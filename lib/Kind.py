...
MAX_WAIT_KIND_NODE_SECONDS = 120
KIND_NODE_INTERVAL_SECONDS = 2

MAX_WAIT_KIND_POD_SECONDS = 150
...
class Kind(common.CommandRunner):
    def create_test_cluster_with_kubernetes_version(self, kube_version):
        ...
        existing_cluster_name = os.getenv(
            'KIND_CLUSTER_'+kube_version.replace('.', '_'))
        if existing_cluster_name:
            logger.console('Using existing kind cluster for ' +
                           kube_version+', "'+existing_cluster_name+'"')
            LAST_CLUSTER_NAME = existing_cluster_name
            LAST_CLUSTER_EXISTING = True
        else:
            new_cluster_name = CLUSTER_PREFIX+'-'+common.NOW+'-'+kube_version
            logger.console('Creating new kind cluster for ' +
                           kube_version+', "'+new_cluster_name+'"')
        ...
        if os.path.exists('/.dockerenv'):
            logger.console('Running in Docker, modifying IP in kubeconfig')
            fixcmd = 'export KIND_IP=$(docker inspect '+LAST_CLUSTER_NAME + \
                '-control-plane | grep \'IPAddress": "\' | head -1 | awk \'{print $2}\' | tr -d \\",) && sed -i "s/https:\/\/127\.0\.0\.1:.*/https:\/\/${KIND_IP}:6443/" $(kind get kubeconfig-path --name="' + \
                LAST_CLUSTER_NAME+'")'
    ...
    def wait_for_cluster(self):
        ...
        while True:
            ...
            if MAX_WAIT_KIND_NODE_SECONDS <= seconds_waited:
                raise Exception(
                    'Max time ('+str(MAX_WAIT_KIND_NODE_SECONDS)+') reached waiting for cluster node')
        ...
        while True:
            ...
            if MAX_WAIT_KIND_POD_SECONDS <= seconds_waited:
                raise Exception(
                    'Max time ('+str(MAX_WAIT_KIND_POD_SECONDS)+') reached waiting for kube-system pods')
